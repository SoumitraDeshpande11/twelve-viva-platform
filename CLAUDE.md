...
```
Config lives in `.env` at **repo root** (copy from `.env.example`). `backend/app/main.py` calls `load_dotenv(parents[2]/.env)` because uvicorn does not load it.
...
- `openai_agent.py`, `gemini_agent.py` — provider-backed equivalents. Each raises `OpenAIAgentError` / `GeminiAgentError` on failure and exposes a `*_configured()` check.
- `gemini_voice.py` — Gemini TTS (question audio WAV) + Gemini Live ephemeral token minting.
- `transcription.py` — server-side audio→text (OpenAI/Gemini/local), provider chosen by `transcription_provider()`.
...
- `selected_ai_provider()` resolves `TWELVE_AI_PROVIDER` (`auto`|`openai`|`gemini`|`local`). `auto` = OpenAI if keyed, else Gemini if keyed, else local.
...
- **Final score is answer-scores only.** Proctoring flags are stored for review and MUST NOT enter score math (`finalize_session()` averages answer score/max_score to a 0–100%).
...
### Frontend
`frontend/lib/api.ts` is the single typed API client (`API_BASE` from `NEXT_PUBLIC_API_BASE`, credentials included for cookies). Pages: `app/admin`, `app/student`, `app/review`, with `AuthPanel.tsx` for staff login. Voice path is hybrid: Gemini TTS WAV for questions (fallback browser `speechSynthesis`), browser `SpeechRecognition` draft + server-side transcription of uploaded audio as the scored source of truth.
