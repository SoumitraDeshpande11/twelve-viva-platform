...
import { API_BASE, api, VivaAnswer, VivaSession } from "../../lib/api";
...
export type Recording = { id: string; ref: string; size_bytes: number; created_at: string };

export type ReviewSession = VivaSession & {
...
/**
 * In-depth AI scoring breakdown: expected-point coverage, per-rubric-criterion
 * assessment, and examiner concerns. Renders only the parts the scorer actually
 * populated, so the deterministic local fallback (reasoning-only) stays clean.
 */
export function AnswerBreakdown({ answer }: { answer: VivaAnswer }) {
  const covered = answer.expected_points_covered ?? [];
  const missed = answer.expected_points_missed ?? [];
  const concerns = answer.concerns ?? [];
  const breakdown = Object.entries(
    (answer.rubric_breakdown as Record<string, unknown> | undefined) ?? {}
  ).filter(([, v]) => typeof v === "string" && v.trim());

  if (!covered.length && !missed.length && !concerns.length && !breakdown.length) {
    return null;
  }

  return (
    <div className="mt-3 space-y-3 rounded-[var(--radius-control)] border border-line bg-surface-2/40 p-3">
      {breakdown.length > 0 && (
        <div>
          <p className="text-[0.66rem] font-medium uppercase tracking-[0.16em] text-muted">
            Rubric breakdown
          </p>
          <ul className="mt-1.5 space-y-1">
            {breakdown.map(([criterion, assessment]) => (
              <li key={criterion} className="text-[0.78rem] leading-snug text-ink">
                <span className="font-medium">{criterion}:</span>{" "}
                <span className="text-ink-soft">{String(assessment)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {(covered.length > 0 || missed.length > 0) && (
        <div className="grid gap-3 sm:grid-cols-2">
          {covered.length > 0 && (
            <div>
              <p className="text-[0.66rem] font-medium uppercase tracking-[0.16em] text-emerald-600 dark:text-emerald-400">
                Points covered
              </p>
              <ul className="mt-1.5 list-disc space-y-0.5 pl-4 text-[0.78rem] leading-snug text-ink-soft">
                {covered.map((p, i) => (
                  <li key={i}>{p}</li>
                ))}
              </ul>
          ...
          {missed.length > 0 && (
            <div>
              <p className="text-[0.66rem] font-medium uppercase tracking-[0.16em] text-amber-600 dark:text-amber-400">
                Points missed
              </p>
              <ul className="mt-1.5 list-disc space-y-0.5 pl-4 text-[0.78rem] leading-snug text-ink-soft">
                {missed.map((p, i) => (
      ...
      {concerns.length > 0 && (
        <div>
          <p className="text-[0.66rem] font-medium uppercase tracking-[0.16em] text-danger">
            Concerns
          </p>
          <ul className="mt-1.5 list-disc space-y-0.5 pl-4 text-[0.78rem] leading-snug text-ink-soft">
            {concerns.map((c, i) => (
...
/**
 * Full session record for a single student attempt: header + score, and the
 * Summary / Answers / Proctoring / Recording / Transcript tabs, plus the score
 * override form and re-score / re-transcribe actions. Self-contained — it fetches
 * the session by id and refreshes itself after mutations.
 */
export function SessionDetail({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    ...
    api<ReviewSession>(`/api/review/sessions/${sessionId}`)
      .then((data) => {
  ...
  if (!selected) {
    return (
      <Banner tone="danger">{message || "Failed to load session."}</Banner>
    );
  }

  // Keep null when genuinely pending so ScorePill renders "—/pending" instead of "0%".
  const effective = selected.effective_score ?? selected.final_score ?? null;
